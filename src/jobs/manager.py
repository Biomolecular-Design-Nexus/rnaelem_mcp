#!/usr/bin/env python3
"""Job management for long-running RNAelem tasks."""
...
class JobManager:
            ...
            try:
                ...
                for key, value in args.items():
                    if value is not None:
                        if key == "input_files":
                            # Handle list of input files for batch processing
                            cmd.extend([f"--{key}", ",".join(value) if isinstance(value, list) else str(value)])
                        else:
                            cmd.extend([f"--{key}", str(value)])

                # Set output paths for structured output
                output_file = job_dir / "output.json"
                if "output_dir" not in args and "output" not in args:
                    cmd.extend(["--output", str(job_dir)])
                ...
                if process.returncode == 0:
                    ...
                    # Try to collect results from script output
                    self._collect_job_results(job_id, job_dir, args)
                else:
    ...
    def _collect_job_results(self, job_id: str, job_dir: Path, args: Dict):
        """Collect results from completed job."""
        # Look for output files in various locations
        output_locations = [
            job_dir / "output.json",
            job_dir / "result.json",
        ...
        # If output_dir was specified, also check there
        ...
        # Collect any files that exist
        result_files = {}
        for output_file in output_locations:
            if output_file.exists():
                try:
                    with open(output_file) as f:
                        result_files[output_file.name] = json.load(f)
                except json.JSONDecodeError:
                    # If not JSON, store as text
                    with open(output_file) as f:
                        result_files[output_file.name] = f.read()

        # Save collected results
        if result_files:
            results_file = job_dir / "collected_results.json"
            with open(results_file, 'w') as f:
                json.dump(result_files, f, indent=2)
    ...
    def get_job_result(self, job_id: str) -> Dict[str, Any]:
        ...
        # Load output - try multiple sources
        job_dir = self.jobs_dir / job_id

        # First try collected results
        collected_results = job_dir / "collected_results.json"
        if collected_results.exists():
            with open(collected_results) as f:
                result = json.load(f)
            return {"status": "success", "result": result}

        # Fallback to checking common output files
        output_files = [
            job_dir / "output.json",
            job_dir / "result.json"
        ]
        ...
        # If no structured output, return job directory contents
        return {
            "status": "success",
            "result": {
                "job_directory": str(job_dir),
                "output_files": [f.name for f in job_dir.iterdir() if f.is_file()]
            }
        }
